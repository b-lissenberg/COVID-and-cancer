* Encoding: UTF-8.
** multivariable voor scale=13 (continue PRO).
USE ALL. 
COMPUTE filter_$=(scale=13).
FILTER BY filter_$.
EXECUTE.
CROSSTABS /TABLES=fam04a_cat BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
...
* living alone: klb_bmi.24 (p=0.040), WHO_dich (p=0.030).
...
* klb_bmi.24 nog steeds effect modificator, maar sample size groot genoeg?.
CROSSTABS /TABLES=Nklb_bmi BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
