* Encoding: UTF-8.
** multivariable voor scale=15 (continue PRO).
USE ALL. 
COMPUTE filter_$=(scale=15).
FILTER BY filter_$.
EXECUTE.
CROSSTABS /TABLES=WHO_dich statusM24 M24intent BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
* M24intent te kleine sample size voor verdere stratificatie.

*  univariaat sterkte modificator: statusM24.
...
* residual/recurrece/SP: fam04b_cat (p=0.026), klb_bmi.24 (p=0.009)
* sample sizes voor deze dubbele stratificatie?.
CROSSTABS /TABLES=fam04b_cat BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
* fam04b_cat te kleine sample size voor stratificatie, en ook voor klb_bmi.24 zou de sample size te klein zijn voor verdere stratificatie bij median split.
