* Encoding: UTF-8.
** multivariable voor scale=24 (continue PRO).
USE ALL. 
COMPUTE filter_$=(scale=24).
FILTER BY filter_$.
EXECUTE.
CROSSTABS /TABLES=edu01_cat stage_1 cidep11.24 BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
...
* dan is klb_bmi.24 geen effect modificator meer.
...
* no lifetime dep: klb_bmi.24 (p=0.047).    
* klb_bmi.24 is continue modificator -> median split voor stratificatie.
MIXED outcome BY metingR groep Nklb_bmi
  /CRITERIA=DFMETHOD(SATTERTHWAITE) CIN(95) MXITER(100) MXSTEP(10) SCORING(1) SINGULAR(0.000000000001) HCONVERGE(0, ABSOLUTE) LCONVERGE(0, ABSOLUTE) PCONVERGE(0.000001, ABSOLUTE)    
  /FIXED=metingR groep metingR*groep Nklb_bmi groep*Nklb_bmi metingR*Nklb_bmi metingR*groep*Nklb_bmi | SSTYPE(3) /METHOD=ML /PRINT=SOLUTION /RANDOM=INTERCEPT | SUBJECT(pident) COVTYPE(CS) .
