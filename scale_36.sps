* Encoding: UTF-8.
** multivariable voor scale=36 (continue PRO).
USE ALL. 
COMPUTE filter_$=(scale=36).
FILTER BY filter_$.
EXECUTE.
CROSSTABS /TABLES=gender cidep11.24 BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
...
* no lifetime depression: fam04a_cat (p=0.012), fam04b_cat (p=0.003).
* lifetime depression: stage_1 (p=0.035), comorbidity (p=0.004).
* sample sizes voor deze dubbele stratificatie?.
CROSSTABS /TABLES=fam04a_cat fam04b_cat stage_1 comorbidity BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
* fam04a_cat, stage_1 en comorbidity te kleine sample size voor stratificatie.

* binnen no lifetime depression verder stratificeren voor fam04b_cat.
USE ALL. 
COMPUTE filter_$=((scale=36)&(cidep11.24=0)).
FILTER BY filter_$.
EXECUTE.
...
* not married: gender (p=0.034), extrave (p=0.049), klb_bmi.24 (p=0.030).
* married: stage_1 (p=0.022).
* sample sizes voor deze dubbele stratificatie?.
CROSSTABS /TABLES=gender stage_1  BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
* gender, extrave en klb_bmi te kleine sample voor verdere stratificatie.
* stage_1 strate combineren.
...
* binnen no lifetime depression - married verder stratificeren voor stage_2cat.
USE ALL. 
COMPUTE filter_$=((scale=36)&(cidep11.24=0)&(fam04b_cat=1)).
FILTER BY filter_$.
EXECUTE.
...
* stage III/IV: klb_bmi.24 (p=0.004).
* maar te kleine sample voor verdere stratificatie (bij median split).
