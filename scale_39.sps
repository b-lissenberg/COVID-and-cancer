* Encoding: UTF-8.
** multivariable voor scale=39 (continue PRO).
USE ALL. 
COMPUTE filter_$=(scale=39).
FILTER BY filter_$.
EXECUTE.
CROSSTABS /TABLES=tum_site BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
* alleen tum_site 1, 2, 4.
USE ALL. 
COMPUTE filter_$=((scale=39)&((tum_site=1)|(tum_site=2)|(tum_site=4))).
FILTER BY filter_$.
EXECUTE.
...
* dan tum_site geen effect modificator.
USE ALL. 
COMPUTE filter_$=(scale=39).
FILTER BY filter_$.
EXECUTE.
* median split voort iadl_tot.24.
MIXED outcome BY metingR groep Niadl_to
  /CRITERIA=DFMETHOD(SATTERTHWAITE) CIN(95) MXITER(100) MXSTEP(10) SCORING(1) SINGULAR(0.000000000001) HCONVERGE(0, ABSOLUTE) LCONVERGE(0, ABSOLUTE) PCONVERGE(0.000001, ABSOLUTE)    
  /FIXED=metingR groep metingR*groep Niadl_to groep*Niadl_to metingR*Niadl_to metingR*groep*Niadl_to | SSTYPE(3) /METHOD=ML /PRINT=SOLUTION /RANDOM=INTERCEPT | SUBJECT(pident) COVTYPE(CS) .
* dan iadl_tot.24 nog steeds effect modificator meer.

*  univariaat sterkte modificator: Niadl_to.
SORT CASES BY Niadl_to.
SPLIT FILE LAYERED BY Niadl_to.
...
* iadl_tot.24<median: tum_site (p<0.001).
