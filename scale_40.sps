* Encoding: UTF-8.
** multivariable voor scale=40 (continue PRO).
USE ALL. 
COMPUTE filter_$=(scale=40).
FILTER BY filter_$.
EXECUTE.
CROSSTABS /TABLES=smok_daily.24 statusM24 BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
...
* no residual/recurrence/SP: smok_daily.24 (p=0.036).
* residual/recurrence/SP: comorbidity (p=0.047).
* sample sizes voor deze dubbele stratificatie?.
CROSSTABS /TABLES=smok_daily.24 comorbidity BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
