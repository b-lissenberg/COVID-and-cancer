* Encoding: UTF-8.
** multivariable voor scale=41 (continue PRO).
USE ALL. 
COMPUTE filter_$=(scale=41).
FILTER BY filter_$.
EXECUTE.
CROSSTABS /TABLES=fam04a_cat smok_daily.24 BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
* smok_daily.24 te kleine sample size.
...
* living together: tum_site (p=0.014), comorbidity (p=0.027).
* living alone: tum_site (p=0.037), stage_1 (p=0.002), cidep07.24 (p<0.001).
* sample sizes voor deze dubbele stratificatie?.
CROSSTABS /TABLES=tum_site stage_1 comorbidity cidep07.24 BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
* voor living alone te kleine sample size voor stratificatie.
* voor living together: comorbity te kleine sample voor stratificatie.
* alleen tum_site 1, 2, 4.
USE ALL. 
COMPUTE filter_$=((scale=41)&((tum_site=1)|(tum_site=2)|(tum_site=4))).
FILTER BY filter_$.
EXECUTE.
MIXED outcome BY metingR groep tum_site
  /CRITERIA=DFMETHOD(SATTERTHWAITE) CIN(95) MXITER(100) MXSTEP(10) SCORING(1) SINGULAR(0.000000000001) HCONVERGE(0, ABSOLUTE) LCONVERGE(0, ABSOLUTE) PCONVERGE(0.000001, ABSOLUTE)    
  /FIXED=metingR groep metingR*groep tum_site groep*tum_site metingR*tum_site metingR*groep*tum_site | SSTYPE(3) /METHOD=ML /PRINT=SOLUTION /RANDOM=INTERCEPT | SUBJECT(pident) COVTYPE(CS) .
* living together: tum_site geen effect modificator meer in deze tumor sites.
