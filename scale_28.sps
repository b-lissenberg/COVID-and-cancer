* Encoding: UTF-8.
** multivariable voor scale=28 (continue PRO).
USE ALL. 
COMPUTE filter_$=(scale=28).
FILTER BY filter_$.
EXECUTE.
...
* males: edu01_cat (p=0.040).
* females: stage_1 (p=0.034), alco_excess.24 (p=0.004).
* sample sizes voor deze dubbele stratificatie?.
CROSSTABS /TABLES=edu01_cat stage_1 alco_excess.24 BY groep BY Measurement /FORMAT=AVALUE TABLES /CELLS=COUNT /COUNT ROUND CELL.
* stage_1 en alco_excess.24 kleine sample size voor stratificatie.


*  voor males verder stratificeren naar edu01_cat.
USE ALL. 
COMPUTE filter_$=((scale=28)&(gender=1)).
FILTER BY filter_$.
EXECUTE.

SORT CASES BY edu01_cat.
SPLIT FILE LAYERED BY edu01_cat.
MIXED outcome BY metingR groep WITH Age
  /CRITERIA=DFMETHOD(SATTERTHWAITE) CIN(95) MXITER(100) MXSTEP(10) SCORING(1) SINGULAR(0.000000000001) HCONVERGE(0, ABSOLUTE) LCONVERGE(0, ABSOLUTE) PCONVERGE(0.000001, ABSOLUTE)    
  /FIXED=metingR groep metingR*groep Age groep*Age metingR*Age metingR*groep*Age | SSTYPE(3) /METHOD=ML /PRINT=SOLUTION /RANDOM=INTERCEPT | SUBJECT(pident) COVTYPE(CS) .
MIXED outcome BY metingR groep fam04a_cat
...
* low education level: extrave (p=0.040).
* extrave is continue modificator -> median split voor stratificatie zou te kleine sample opleveren (in totaal 31 males met low educational level)..
